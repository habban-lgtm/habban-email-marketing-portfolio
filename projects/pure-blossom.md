# Pure Blossom — Klaviyo Lifecycle Marketing Client Project

**Project type:** Real client project  
**Platform:** Klaviyo  
**Brand:** Pure Blossom — eCommerce brand

## Project Overview

Pure Blossom was a client project focused on building and implementing a structured lifecycle email marketing system in Klaviyo.

The work covered the customer journey from newsletter signup through purchase, repeat engagement, and reactivation. I focused on creating practical automations that matched customer behavior and the brand's communication style.

## Brand & Email Setup

I created and configured the email marketing foundation for Pure Blossom in Klaviyo.

**Included:**
- Logo and brand colors
- Email typography
- Button styling
- Social links
- Brand voice/content direction
- Signup form
- Welcome incentive
- Newsletter list setup

The signup form was connected to the **Pure Blossom Newsletter** list with a welcome incentive.

## Lifecycle Flows

### Welcome Flow

**Trigger:** New subscriber joins the newsletter list.

**Sequence:**
1. Welcome / brand introduction
2. Product and value-focused email
3. Offer / purchase incentive

**Purpose:** Introduce the brand, build initial awareness, and encourage the first purchase.

---

### Abandoned Checkout Flow

**Trigger:** Customer starts checkout but does not complete the purchase.

**Sequence:**
1. Delay
2. Reminder email
3. Follow-up email

**Purpose:** Recover potential lost purchases by reminding customers about the products they were considering.

---

### Abandoned Cart Flow

**Trigger:** Customer adds a product to cart but does not complete the transaction.

**Sequence:**
1. Delay
2. Cart reminder email
3. Follow-up reminder

**Purpose:** Re-engage shoppers who showed purchase intent but did not complete the transaction.

---

### Winback Flow

**Trigger:** Customer reaches the defined post-purchase inactivity period.

**Sequence:**
1. Re-engagement message
2. Follow-up communication

**Purpose:** Encourage previous customers to return and make another purchase.

---

### Sunset Flow

**Trigger:** Subscriber enters the sunset audience based on engagement/inactivity criteria.

**Logic:**
- Identify whether the profile has placed an order
- Send a re-engagement sequence
- Provide an opportunity to return
- Move persistently unengaged profiles toward suppression

**Purpose:** Protect list quality while giving inactive subscribers a final opportunity to re-engage.

## What I Implemented

- Klaviyo flow architecture
- Event-based automation
- Customer lifecycle journeys
- Segmentation logic
- Email sequence planning
- eCommerce retention strategy
- Brand-consistent email setup
- Shopify + Klaviyo integration

## Project Scope

My work on this project focused on the **Klaviyo/email marketing side**, including lifecycle strategy, flow setup, segmentation logic, signup capture, and email structure.

## Results & Confidentiality

Performance and revenue figures are not disclosed in this portfolio case study.

This case study focuses on the work I personally implemented and the lifecycle marketing approach used for the client.

## Flow Visual Evidence

A visual overview of the live Pure Blossom lifecycle flows is available in the portfolio assets folder:

- [Pure Blossom Flow Visual Evidence](../assets/pure-blossom-flows/README.md)

**Client privacy note:** Performance results, conversion data, revenue figures, and customer-level information are intentionally not shown in the visual evidence because of client confidentiality and privacy requirements. The visuals are included to demonstrate the flow architecture, lifecycle strategy, automation structure, and implementation work.
